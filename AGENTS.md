...
| `extension/manifest.json` | Permissions, new tab override, toolbar popup, options page. |
| `extension/newtab.html`, `newtab.js` | The new tab page. Synchronous critical path: localStorage cache → iframe. |
| `extension/lib/config.js` | Providers, settings, header rules (declarativeNetRequest), `reconcile()`. |
| `extension/background.js` | Service worker: keeps the rules in sync, opens the welcome page on install. |
| `extension/frame.js` | Content script inside chatgpt.com / claude.ai frames: forwards the page title, icon and background color to the tab. |
| `extension/settings.html`, `popup.html`, `settings.js`, `settings.css` | Options/welcome page and toolbar popup (one shared script). |
| `extension/_locales/{en,fr}/messages.json` | UI strings (`chrome.i18n`). |
...
3. The welcome page opens: they pick ChatGPT or Claude. Done.
...
4. **Least privilege.** Required host permissions are only chatgpt.com and claude.ai. Any other site goes through `optional_host_permissions`, requested at runtime for that single site and removed when it's no longer used. Feature permissions granted inside the frame (microphone, clipboard) are stored for the extension's origin, so custom sites get an empty `allow` list.
5. **No remote code, no analytics, no data collection.** Required by the Chrome Web Store, and promised in `PRIVACY.md`.

## Common changes

- **Add a built-in provider.** Add it to `PROVIDERS` in `extension/lib/config.js` (name, url, background colors, `allow`). Add its origin to `host_permissions` and `content_scripts[0].matches` in `manifest.json`. Add a radio option to `settings.html` and `popup.html`. Run `npm test`.
